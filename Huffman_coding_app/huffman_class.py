...
class HuffmanCoding:
    def __init__(self, path, reverseCodes):
        self.path = path
        self.__heap = []
        self.__codes = {}
        self.__reverseCodes = reverseCodes
    ...
    def __builtHeap(self, freq_dict):
    ...
    def __createZipFile(self, output_path, output_json, file_name):
        print(output_json)
        file_name = file_name + '.textc'
        myzip = zipfile.ZipFile(file_name, 'w')
        myzip.write(output_path)
        myzip.write(output_json)
        print('zip created')
        myzip.close()
        return file_name
    ...
    def compress(self):
        ...
        # read the test form file
        with open(self.path, 'r+') as file, open(output_path, 'wb') as output:
            text = file.read()
            text = text.rstrip()

            # make frequency dictionary from file
            freq_dict = self.__make_frequency_dict(text)
            # construct the heap from the frequency_dict
            self.__builtHeap(freq_dict)
            # Construct the binary tree from heap
            self.__buildTree()
            # construct the codes from binary tree
            self.__buildCodes()
            # Creating new file and coping dict to file
            paths = file_name + "Code.txt"
            f = open(paths, 'w+')
            f.write(str(self.__reverseCodes))
            f.close()
            # creating the encoded text form the codes
            encoded_text = self.__getEncodedText(text)
            # put this encoded text to the binary file
        ...
        zip_file_name = self.__createZipFile(output_path, paths, file_name)
        print("compressed")
        print(zip_file_name)
        return output_path,paths,zip_file_name
